...
namespace SR_UTILS_NS::Localization {
    ...
    public:
        ...
        UnicodeString GetStringById(const StringAtom& id);
    private:
        std::unordered_map<Locale, Path> m_localePaths = { };
        FileWatcher::Ptr m_watchedFile = nullptr;
        std::unordered_map<StringAtom, UnicodeString> m_locStrings = { }; ///first - id, second - UnicodeString for string itself
