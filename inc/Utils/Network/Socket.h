...
namespace SR_NETWORK_NS {
    ...
    public:
        ...
        virtual bool SendTo(const void* data, uint64_t size, StringView address, uint16_t port) = 0;
        SR_NODISCARD virtual uint64_t Receive(void* data, size_t size) = 0;
        SR_NODISCARD virtual uint64_t AsyncReceive(void* data, SR_HTYPES_NS::Function<void(uint64_t bytesReceived)> callback) = 0;
        SR_NODISCARD virtual bool Connect(StringView address, uint16_t port) = 0;
